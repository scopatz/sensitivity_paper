...
\definecolor{lightred}{RGB}{255,94,94}
...
% Article commands
\begin{frontmatter}
\title{Entropy-Based Nuclear Fuel Cycle Sensitivities and Covariances}%\tnoteref{t1}}
\tnotetext[t1]{Number of Pages: 41, Number of Tables: 7, Number of Figures: 7}

\author[ut]{Anthony M. Scopatz\corref{cor1}}
\ead{scopatz@mail.utexas.edu}
...
have been devised.  
\textcolor{red}{One aim of these fuel cycle simulators is to enable}
...
useful, especially when the derivatives of the true function are needed and 
for small numbers of independent variables being modeled.  By its very nature though, 
...
derivatives are not needed, a major benefit of stochastic collocation would go
unused.  Furthermore, due to the large number of fuel cycle parameters possible
the entropy scale well in a fashion that the surrogate models considered do not.
}
...
methodology employed to carry out the fuel cycle study.  \S \ref{sec:nfcsim} 
describes the fuel cycle 
...
space outlined in \S \ref{sec:nfcsim} and offers several case studies that 
illustrate the interpretation
of the statistical outcomes.  \S \ref{sec:conclusion} concludes with options 
for future work.
...
fresh fuel compositions.  Since the burnup, criticality, and transmutation information is parameterized per
nuclide, this essential physics reactor submodel is more precise and perturbable than full-core linear 
correlation coefficients.  However since the neutron flux spectrum is not parameterized, this reactor 
methodology assumes a static flux over the course of the burn.  This paper is limited to examining cores where 
static spectra are a reasonable approximations.
...
\begin{figure}[htbp]
\begin{center}
\colorbox{lightred}{\includegraphics[scale=0.50]{LWR_FR.eps}}
\textcolor{red}{\caption{LWR-FR Symbiotic Fuel Cycle Scenario}}
\label{lwr_fr_fc}
\end{center}
\end{figure}
...
period, is sent to an aqueous reprocessing facility. 
\textcolor{red}{
Here the transuranics (TRU) are separated out
from the fission products (FP) and
the used uranium (UU).  The FP stream is further partitioned at the
...
isotopes while the UU goes to a low-level waste (LLW) disposal facility.
}
...
In the case where a set of input parameters is chosen such that the cycle is not physically 
\textcolor{red}{calculable},
...
internal parameters.  
\textcolor{red}{
The reactor model will adjust the non-leakage probability, the initial enrichment
...
\textcolor{red}{
Note that the min and max bounds
...
loop of Figure \ref{mcmethod} is entered. 
Since all inputs are varied simultaneously, 
the response from any given input parameter is implicitly a function of all other 
input parameters as well.
} 
With thirty inputs, all outcomes exist in a 31 dimensional 
space.
...
This section describes the entropy-based statistical measures used to quantify 
the strength of association between input parameters and responses in contingency tables.
...
\textcolor{red}{
Note that these are fundamentally distinct from the Monte Carlo statistics 
which may be computed. For the input parameters, the standard deviation $\sigma(x)$
is determined by the sampling function for $x$. (Inputs sampled via a 
linearly-uniform probability distribution have the well known standard deviation
$\sigma(x) = (x_{\max} - x_{\min})/2\sqrt(3)$.)  Furthermore, the standard deviation 
of the response $\sigma(R)$ is the same for all inputs.  This can be seen in Tables
\ref{ct2d_example} \& \ref{ct3d_example} where the marginal sum in right-most column, 
the distribution of $R$ summed over all $x$, is the same.  For these tables
$\sigma(R)\approx 5.6\times 10^6$ where $R$ is the repository capacity.
}

\textcolor{red}{
Because $\sigma(x)$ and $\sigma(R)$ are both independent of $R(x)$, a
more appropriate statistic would be the conditional variance $\sigma^2(x|R)$.  
However, this is not a traditional stochastic metric.  Instead the entropy-based
measures seen below - for which $R(x)$ need not be explicitly known - take the 
place of such a conditional variance.
}
...
densely populated region, adding information about the distribution that was not available under the more coarsely-binned structure.
...
%\input{U_x_R_for_Capacity_with_IJK7}
\include{U_x_R_for_Capacity_with_IJK7}

As expected, the fast reactor plutonium separation efficiency is shown to be the driver of the system.  
\textcolor{red}{
How much more important this parameter is than the
other inputs may be quancp titatively stated by taking the ratio of the two $U(x|R)$.  
}
...
%\input{U_xy_R_for_Capacity_with_IJK7}
...
%\input{cv_x_y_R_for_Capacity_with_IJK7}
\include{cv_x_y_R_for_Capacity_with_IJK7}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.70]{FR_SE_AM_and_FR_SE_PU_Decay_Heat.eps}
...
%\input{FR_SE_AM_and_FR_TRU_CR_to_Capacity_with_IJK433_prop}
\include{FR_SE_AM_and_FR_TRU_CR_to_Capacity_with_IJK433_prop}
...
there are simply fewer LWRs.  
\textcolor{red}{
Specifically, the relative impact of \texttt{FR\_SE\_AM} increases.
}
...
Through an examination of the the 3D contingency table, high repository capacities are only allowed when both \texttt{FR\_SE\_PU} and
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.70]{FR_SE_PU_and_HLW_Storage_Time_Decay_Heat.eps}
...
By having a long \texttt{HLW\_Storage\_Time}, the short- and medium-term repository temperature limits may be avoided.
...
the inverse problem.  Conditioned on a specific system response, 
like repository capacity, the measure of the strength of response provided by the uncertainty coefficients can provide guidance to technologists optimizing design parameters in the face of uncertainty regarding the shape of the larger system.  Illuminating joint dependencies, coefficients of variation can guide iterative technology development by showing which design features must be considered together.

\textcolor{red}{
The approach described here can be generalized to include boolean as well as 
continuous inputs.  For example, if the technology ultimately chosen for the 
permanent repository is unknown, a boolean flag can allow each stochastic 
realization to follow one of several options.  The outcome, still repository 
capacity, would now be conditioned on this boolean input as well.  Note that 
responses to a boolean input parameters, like their categorical or continuous 
counterparts, may be just as sensitive to the probability distribution function 
chosen for them (Table \ref{param_def}).
Still, boolean variables would illuminate system design parameters that will be 
important regardless of technology ultimately chosen, those that have strongly 
technology-dependent effects, and those that are unimportant to determining 
capacity regardless of technology.  In view of the uncertainties in the 
direction taken by nuclear energy in the future, coupled with the need to 
develop technologies now that can support or even enable these outcomes, 
this capability, if developed, could provide invaluable guidance.
}
...
Jacob~J Jacobson, Gretchen~E Matthern, Steven~J Piet, and David~E Shropshire.
\newblock {VISION: Verifiable Fuel Cycle Simulation Model Advances in Nuclear Fuel Management}.
\newblock {\em Management}, 2009.

\bibitem{GENIUS1}
Kyle~Matthew Oliver, Paul P H~Wilson {andc Arnaud Reveillere}, Tae~Wook Ahn,
  Kerry Dunn, Katy Huff, and Royal Elmore.
\newblock {Studying international fuel cycle robustness with the GENIUSv2
  discrete facilities/materials fuel cycle systems analysis tool}.
\newblock In {\em GLOBAL 2009}, 2009.

\bibitem{Mathelin03}
Lionel Mathelin and M. Yousuff Hussaini.
\newblock {A Stochastic Collocation Algorithm for Uncertainty Analysis}.
\newblock {\em NASA}, 2003--212153, 2003.

\bibitem{Shannon1948}
C~E Shannon.
\newblock {A Mathematical Theory of Communication}.
\newblock {\em M.D. computing : computers in medical practice}, 14(4):306--17,
  1948.
...
\newblock {Weitere studien \"{u}ber das w\"{a}rmegleichgewicht unter
  gasmolek\"{u}len}.
\newblock {\em Sitzungsberichte Akademie der Wissenschaften}, 66:275--370,
  1872.

\bibitem{Watanabe1960}
Satosi Watanabe.
\newblock {Information Theoretical Analysis of Multivariate Correlation}.
\newblock {\em IBM Journal of Research and Development}, 4(1):66--82, January
  1960.

\bibitem{Rashevsky1955}
N.~Rashevsky.
\newblock {Life, information theory, and topology}.
\newblock {\em The Bulletin of Mathematical Biophysics}, 17(3):229--235,
  September 1955.

\bibitem{Yockey2011}
Hubert~P. Yockey.
\newblock {\em {Information Theory, Evolution, and The Origin of Life}}.
...
\newblock {Common randomness in information theory and cryptography. I. Secret
  sharing}.
\newblock {\em IEEE Transactions on Information Theory}, 39(4):1121--1132, July
  1993.

\bibitem{Pearson1904}
Karl Pearson.
\newblock {\em {On the theory of contingency and its relation to association
  and normal correlation.}}
\newblock Dulau and Co., London, 1904.

\bibitem{Liu2006}
Huibin Liu, Wei Chen, and Agus Sudjianto.
\newblock {Relative Entropy Based Method for Probabilistic Sensitivity Analysis
  in Engineering Design}.
\newblock {\em Journal of Mechanical Design}, 128(2):326, 2006.

\bibitem{Scopatz2009}
Anthony~M. Scopatz and Erich~a. Schneider.
\newblock {A new method for rapid computation of transient fuel cycle material
  balances}.
\newblock {\em Nuclear Engineering and Design}, 239(10):2169--2184, October
  2009.

\bibitem{Li2009}
Jun Li, Man-sung Yim, and D~N McNelis.
\newblock {An Efficient and Comprehensive Model for Fuel Cycle System
  Performance Evaluation}.
\newblock In {\em GLOBAL 2009}, Paris, France, 2009.

\bibitem{Li2009b}
J~Li, A~M Scopatz, M~S Yim, and E~A Schneider.
\newblock {The Sensitivity of Fuel Cycle Performance to Separation Efficiency}.
\newblock {\em Nuclear Engineering and Design}, 240:511--523, 2009.

\bibitem{Li2010a}
Jun Li, Man-sung Yim, and D~N McNelis.
\newblock {A Simplified Methodology for Nuclear Waste Repository Thermal
  Analysis}.
\newblock {\em Annals of Nuclear Energy}, 2010.

\bibitem{NEA-5990}
OECD.
\newblock {Advanced Nuclear Fuel Cycles and Radioactive Waste Management -
  NEA-5990}.
\newblock Technical report, Nuclear Energy Agency, 2006.

\bibitem{Shropshire2009}
D~E Shropshire, K~A Williams, E~A Hoffman, J~D Smith, D~J Hebditch, J~J
  Jacobson, J~D Morton, A~M Phillips, and J~P Taylor.
\newblock {Advanced Fuel Cycle Economic Analysis of Symbiotic Light-Water
  Reactor and Fast Burner Reactor Systems - INL/EXT-09-15254}.
\newblock Technical Report January, Idaho National Labortory, Idaho Falls, ID,
  USA, 2009.

\bibitem{Everitt1992}
Brian~S. Everitt.
\newblock {\em {Analysis of Contingency Tables}}.
\newblock Chapman \& Hall, 1992.

\bibitem{Press2007}
William~H Press, Saul~A Teukolsky, William~T Vetterling, and Brian~P Flannery.
\newblock {\em {Numerical Recipes 3rd Edition: The Art of Scientific
  Computing}}.
\newblock Cambridge University Press, 2007.

\bibitem{Yao2003}
YY~Yao and S.~Regina.
\newblock {6 Information-Theoretic Measures for Knowledge Discovery and Data
  Mining}.
\newblock {\em Entropy measures, maximum entropy principle and emerging
  applications}, page 115, 2003.

\bibitem{Scopatz2010b}
Anthony Scopatz, Jun Li, Man-sung Yim, and Erich Schneider.
\newblock {Nuclear Fuel Cycle Sensitivity to the Variation of Physical
  Parameters}.
\newblock In {\em PHYSOR 2010}, Pittsburgh, PA, USA, 2010.

\bibitem{Barratt2004}
J.G. Barratt and E.A. Schneider.
\newblock {Monte Carlo Uncertainty Analysis of a Transmuting Fuel Cycle}.
\newblock {\em TRANSACTIONS-AMERICAN NUCLEAR SOCIETY}, 90:249--250, 2004.

\bibitem{Scopatz2009c}
Anthony Scopatz, Jun Li, Man-sung Yim, and Erich Schneider.
\newblock {A Systemic Evaluation of the Sensitivity of Fuel Cycle}.
\newblock In {\em GLOBAL 2009}, Paris, France, 2009.
